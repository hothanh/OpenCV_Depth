...
int main(int argc, char** argv)
{
	...
	cv::CommandLineParser parser(argc, argv, "{imagemode||}{left||}{right||}{help h||}{algorithm||}{max-disparity|0|}{blocksize|0|}{scale|1|}{i||}{e||}{o||}{od||}{l||}{r||}");
	...
	if (parser.has("l"))
	{	lfolder = parser.get<std::string>("l");
	}
	if (parser.has("r"))
	{
		rfolder = parser.get<std::string>("r");
	}
	int img_mode = 0;
	...
	if(img_mode == 0)
	{
		...
		img1_filename = "stereoImg/left_image_338.png";
		img2_filename = "stereoImg/right_image_338.png";
		img1 = imread(img1_filename, color_mode);
		img2 = imread(img2_filename, color_mode);
		cout << img1_filename << ": " << img1.size() << endl;
		cout << img2_filename << ": " << img2.size() << endl;
	...
	for(int i = 1; i < 5000; i++)
	{
		if (img_mode == 1) 
		{
			...
			//strleft << "stereoImg/left_image_" << i << ".png";
			strleft << lfolder << "image" << i << ".png";
			//strright << "stereoImg/right_image_" << i << ".png";
			strright << rfolder << "image" << i << ".png";
		...
		{
			stringstream str1;
			str1 << lfolder << "../result/depth_bin/depth" << i << ".matbin";
		...
		//imgDepth32f.convertTo(imgDepth16S, CV_16S);
		// << ****** just checking if depth map saved correctly and loaded as it is ****** >>

		imgDisparity32F.convertTo(imgDepth16S, CV_16S);
...
//		imshow("disparity-map", filtered_disp_vis);
		...
		if (img_mode == 0) {
			...
			stringstream str1;
			str1 << lfolder << "../result/dispa_image/dispa" << i << ".png";
